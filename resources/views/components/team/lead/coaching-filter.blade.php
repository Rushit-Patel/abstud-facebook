...
<x-team.drawer.drawer id="{{ $id }}" title="{{ $title }}">
    <x-slot name="body">
        <form id="coachingFilterForm" method="POST" >
            @csrf
            @method('POST')
            {{-- Date Range Filter --}}
    ...
    <x-slot name="footer">
        <button type="button" class="kt-btn kt-btn-outline" onclick="resetCoachingFilters()">
            Reset
        </button>
        <button type="submit" form="coachingFilterForm" class="kt-btn kt-btn-primary">
            Apply Filters
        </button>
    </x-slot>
