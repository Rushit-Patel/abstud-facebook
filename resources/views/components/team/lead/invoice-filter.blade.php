...
<x-team.drawer.drawer id="{{ $id }}" title="{{ $title }}">
    <x-slot name="body">
        <form id="invoiceFilterForm" method="POST" >
            ...
            @haspermission('invoice:show-all')
            ...
            @haspermission('invoice:show-branch')
            <div class="border-b border-border mb-4 mt-5"></div>
    ...
    <x-slot name="footer">
        <button type="button" class="kt-btn kt-btn-outline" onclick="resetInvoiceFilters()">
            Reset
        </button>
        <button type="submit" form="invoiceFilterForm" class="kt-btn kt-btn-primary">
            Apply Filters
        </button>
    </x-slot>
