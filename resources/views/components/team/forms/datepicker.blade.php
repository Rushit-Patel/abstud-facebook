...
<div class="flex flex-col gap-1.5">
    ...
    <div class="relative">
        <input 
            ...
            class=" @if($enableTime) timepickr @else flatpickr @endif {{ $classes }}"
            @if($required) required @endif
            @if($readonly) readonly @endif
            @if($disabled) disabled @endif
            data-flatpickr
            data-date-format="{{ $dateFormat }}"
            @if($minDate) data-min_date="{{ $minDate }}" @endif
            @if($maxDate) data-max_date="{{ $maxDate }}" @endif
            @if($minTime) data-min_time="{{ $minTime }}" @endif
            @if($maxTime) data-max_time="{{ $maxTime }}" @endif
