...
<x-team.layout.app title="Edit Lead Status" :breadcrumbs="$breadcrumbs">
    <x-slot name="content">
        <div class="kt-container-fixed">
            <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
                <div class="flex flex-col justify-center gap-2">
                    <h1 class="text-xl font-medium leading-none text-mono">
                        Edit Lead Status: {{ $LeadStatus->name }}
                    </h1>
            ...
            <x-team.card title="Lead Status Information" headerClass="">
                <form action="{{ route('team.settings.lead-status.update', $LeadStatus) }}" method="POST" class="form">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="flex flex-col gap-1 mt-4">
                                    ...
                                    <label class="kt-label">
                                        <input class="kt-checkbox kt-checkbox-sm"
                                            ...
                                            type="checkbox"
                                            value="1"
                                            {{ old('status', $LeadStatus->status) ? 'checked' : '' }}
                                        />
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 py-5 border-t border-gray-200">
                        <div class="flex flex-col gap-2.5">
                            ...
                            <div class="text-sm text-gray-700">
                                {{ $LeadStatus->created_at->format('M d, Y \a\t h:i A') }}
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-2.5">
                            ...
                            <div class="text-sm text-gray-700">
                                {{ $LeadStatus->updated_at->format('M d, Y \a\t h:i A') }}
                            </div>
                        </div>
                    </div>
