   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('education_level', fn($row) => $this->renderEducationLevel($row))
            ->filterColumn('education_level', function($query, $keyword) {
                $query->where('name', 'like', "%{$keyword}%");
            })
            ->addColumn('status', fn($row) => $this->renderStatus($row))
            ->rawColumns(['action', 'education_level', 'status'])
            ->setRowId('id');
    }

    public function query(EducationLevel $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('education-level-table')
    ...
    {
        return [
            Column::computed('education_level')->title('Education Level')->exportable(false)->printable(true)->width(200)->addClass('text-start')->searchable(true),
    ...
    {
        $button = '<a href="'.route('team.settings.education-level.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.education-level.destroy', $row->id) . '">
    ...
    protected function renderEducationLevel($row): string
    {
        return  e($row->name);
    }
