...
class CoachingLengthDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('name', fn($row) => $this->renderCoachingLength($row))
    ...
    public function query(CoachingLength $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('coaching-length-table')
    ...
    {
        return 'CoachingLength_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.coaching-length.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.coaching-length.destroy', $row->id) . '">
    ...
    protected function renderCoachingLength($row): string
    {
        return  e($row->name);
    }
