...
class TypeOfRelativeDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('type_of_relative', fn($row) => $this->renderTypeOfRelative($row))
            ->filterColumn('type_of_relative', function($query, $keyword) {
                $query->where('name', 'like', "%{$keyword}%");
            })
            ->addColumn('status', fn($row) => $this->renderStatus($row))
            ->rawColumns(['action', 'type_of_relative', 'status'])
            ->setRowId('id');
    }

    public function query(TypeOfRelative $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('type_of_relative-table')
    ...
    {
        return [
            Column::computed('type_of_relative')->title('Type Of Relative')->exportable(false)->printable(true)->width(200)->addClass('text-start')->searchable(true),
    ...
    {
        return 'TypeOfRelative_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.type-of-relative.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.type-of-relative.destroy', $row->id) . '">
    ...
    protected function renderTypeOfRelative($row): string
    {
        return  e($row->name);
    }
