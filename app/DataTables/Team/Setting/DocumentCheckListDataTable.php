...
class DocumentCheckListDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('name', fn($row) => $this->renderDocumentCheckList($row))
    ...
    public function query(DocumentCheckList $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('document-check-list-table')
    ...
    {
        return 'DocumentCheckList_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.document-check-list.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.document-check-list.destroy', $row->id) . '">
    ...
    protected function renderDocumentCheckList($row): string
    {
        return  e($row->name);
    }
