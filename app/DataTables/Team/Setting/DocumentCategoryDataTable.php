   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('name', fn($row) => $this->renderDocumentCategory($row))
    ...
    {
        $button = '<a href="'.route('team.settings.document-category.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.document-category.destroy', $row->id) . '">
    ...
    protected function renderDocumentCategory($row): string
    {
        return  e($row->name);
    }
