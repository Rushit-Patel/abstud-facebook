...
class OtherVisaTypeDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('visa_type', fn($row) => $this->renderOtherVisaType($row))
            ->filterColumn('visa_type', function($query, $keyword) {
                $query->where('name', 'like', "%{$keyword}%");
            })
            ->addColumn('status', fn($row) => $this->renderStatus($row))
            ->rawColumns(['action', 'visa_type', 'status'])
            ->setRowId('id');
    }

    public function query(OtherVisaType $model): QueryBuilder
    {
    ...
    {
        return [
            Column::computed('visa_type')->title('Visa Type')->exportable(false)->printable(true)->width(200)->addClass('text-start')->searchable(true),
    ...
    {
        return 'OtherVisaType_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.other-visa-type.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.other-visa-type.destroy', $row->id) . '">
    ...
    protected function renderOtherVisaType($row): string
    {
        return  e($row->name);
    }
