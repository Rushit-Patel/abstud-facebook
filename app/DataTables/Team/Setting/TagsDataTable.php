   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tag', fn($row) => $this->renderTags($row))
            ->filterColumn('tag', function($query, $keyword) {
                $query->where('name', 'like', "%{$keyword}%");
            })
            ->addColumn('status', fn($row) => $this->renderStatus($row))
            ->rawColumns(['action', 'tag', 'status'])
            ->setRowId('id');
    }

    public function query(LeadTag $model): QueryBuilder
    {
    ...
    {
        return [
            Column::computed('tag')->title('Lead Tag')->exportable(false)->printable(true)->width(200)->addClass('text-start')->searchable(true),
    ...
    {
        $button = '<a href="'.route('team.settings.tags.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.tags.destroy', $row->id) . '">
    ...
    protected function renderTags($row): string
    {
        return  e($row->name);
    }
