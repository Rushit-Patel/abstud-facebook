...
class EducationBoardDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('education_board', fn($row) => $this->renderEducationBoard($row))
            ->filterColumn('education_board', function($query, $keyword) {
                $query->where('name', 'like', "%{$keyword}%");
            })
            ->addColumn('status', fn($row) => $this->renderStatus($row))
            ->rawColumns(['action', 'education_board', 'status'])
            ->setRowId('id');
    }

    public function query(EducationBoard $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('education-board-table')
    ...
    {
        return [
            Column::computed('education_board')->title('Education Board')->exportable(false)->printable(true)->width(200)->addClass('text-start')->searchable(true),
    ...
    {
        return 'EducationBoard_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.education-board.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.education-board.destroy', $row->id) . '">
    ...
    protected function renderEducationBoard($row): string
    {
        return  e($row->name);
    }
