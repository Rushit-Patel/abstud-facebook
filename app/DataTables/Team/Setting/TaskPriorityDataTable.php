...
class TaskPriorityDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('name', fn($row) => $this->renderTaskPriority($row))
    ...
    public function query(TaskPriority $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('Task-Priority-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('is_active')->title('Status')->width(130),
    ...
    {
        return 'TaskPriority_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.task-priority.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.task-priority.destroy', $row->id) . '">
    ...
    protected function renderTaskPriority($row): string
    {
        return  e($row->name);
    }
