...
class ExamModeDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('name', fn($row) => $this->renderExamMode($row))
    ...
    public function query(ExamMode $model): QueryBuilder
    {
    ...
    {
        return 'ExamMode_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.exam-mode.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.exam-mode.destroy', $row->id) . '">
    ...
    protected function renderExamMode($row): string
    {
        return  e($row->name);
    }
