...
class ExamWayDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('name', fn($row) => $this->renderExamWay($row))
    ...
    public function query(ExamWay $model): QueryBuilder
    {
    ...
    {
        return 'ExamWay_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.exam-way.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.exam-way.destroy', $row->id) . '">
    ...
    protected function renderExamWay($row): string
    {
        return  e($row->name);
    }
