...
class ForeignStateDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    ...
    public function query(ForeignState $model): QueryBuilder
    {
    ...
    {
        return 'ForeignState_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.foreign-state.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.foreign-state.destroy', $row->id) . '">
    ...
    protected function renderForeignState($row): string
    {
        return  e($row->name);
    }
    ...
    {
        // dd($row->country_id, $row->getCountry->name);
        return e($row->getCountry->name);
    }
