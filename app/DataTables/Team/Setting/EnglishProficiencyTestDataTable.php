...
class EnglishProficiencyTestDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('name', fn($row) => $this->renderEnglishProficiencyTest($row))
    ...
    public function query(EnglishProficiencyTest $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('english-proficiency-test-table')
    ...
    {
        return 'EnglishProficiencyTest_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.english-proficiency-test.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.english-proficiency-test.destroy', $row->id) . '">
    ...
    protected function renderEnglishProficiencyTest($row): string
    {
        return  e($row->name);
    }
