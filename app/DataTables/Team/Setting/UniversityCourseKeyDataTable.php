...
class UniversityCourseKeyDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', fn($row) => $this->renderAction($row))
            ->addColumn('name', fn($row) => $this->renderUniversityCourseKey($row))
            ->filterColumn('name', function($query, $keyword) {
                $query->where('name', 'like', "%{$keyword}%");
            })
            ->addColumn('title', fn($row) => $this->renderUniversityCourseKeyTitle($row))
    ...
    public function query(UniversityCourseKey $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('UniversityCourseKey-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('title')->title('Title')->exportable(false)->printable(true)->width(200)->addClass('text-start')->searchable(true),
    ...
    {
        return 'UniversityCourseKey_' . date('YmdHis');
    }
    ...
    {
        $button = '
            <a href="'.route('team.settings.university-course-key.edit', $row->id).'" class="btn btn-sm btn-primary">
                <i class="ki-filled ki-notepad-edit text-2xl me-2"></i>
            </a>
        ';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.university-course-key.destroy', $row->id) . '">
    ...
    protected function renderUniversityCourseKey($row): string
    {
        return $row->name;
    }

    protected function renderUniversityCourseKeyTitle($row): string
    {
        return $row->title;
    }
