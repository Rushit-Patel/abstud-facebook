   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('image', fn($row) => $this->renderImage($row))
            ->addColumn('status', fn($row) => $this->renderStatus($row))
            ->rawColumns(['action', 'purpose', 'image', 'status'])
            ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('image')->title('Image')->exportable(false)->printable(false)->width(100)->addClass('text-center')->orderable(false)->searchable(false),
    ...
    protected function renderImage($row): string
    {
        if ($row->image) {
            return '<div class="w-12 h-12 rounded-lg overflow-hidden border border-input mx-auto">
        ...
        return '<div class="w-12 h-12 rounded-lg border-2 border-dashed border-input flex items-center justify-center bg-background mx-auto">
