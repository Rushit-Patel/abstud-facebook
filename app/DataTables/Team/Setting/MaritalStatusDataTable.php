   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('marital_status', fn($row) => $this->renderMaritalStatus($row))
            ->filterColumn('marital_status', function($query, $keyword) {
                $query->where('name', 'like', "%{$keyword}%");
            })
            ->addColumn('status', fn($row) => $this->renderStatus($row))
            ->rawColumns(['action', 'marital_status', 'status'])
            ->setRowId('id');
    }
    ...
    {
        return $this->builder()
            ->setTableId('marital_status-table')
    ...
    {
        return [
            Column::computed('marital_status')->title('Marital Status')->exportable(false)->printable(true)->width(200)->addClass('text-start')->searchable(true),
    ...
    {
        $button = '<a href="'.route('team.settings.marital-status.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.marital-status.destroy', $row->id) . '">
    ...
    protected function renderMaritalStatus($row): string
    {
        return  e($row->name);
    }
