...
class ForeignCityDataTable extends DataTable
{
    /**
   ...
   public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('city', fn($row) => $this->renderForeignCity($row))
            ...
            ->rawColumns(['action', 'city', 'status'])
            ->setRowId('id');
    }

    public function query(ForeignCity $model): QueryBuilder
    {
    ...
    {
        return 'ForeignCity_' . date('YmdHis');
    }

    // Dummy render methods for customization
    protected function renderAction($row): string
    {
        $button = '<a href="'.route('team.settings.foreign-city.edit', $row->id).'" class="btn btn-sm btn-primary"><i class="ki-filled ki-notepad-edit text-2xl me-2"></i></a>';
        $deleteBtn = '
            <button type="delete" data-kt-modal-toggle="#delete_modal" data-form_action="' . route('team.settings.foreign-city.destroy', $row->id) . '">
    ...
    protected function renderForeignCity($row): string
    {
        return  e($row->name);
    }
