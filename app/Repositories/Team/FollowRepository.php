...
{
    public static function getFollowUp()
    {
        $main_query = LeadFollowUp::with('clientLead');
    ...
    /**
     * Apply branch-specific filtering logic for follow-ups
     */
    private static function applyBranchFilters($query, $user)
    {
        ...
        // Check all user roles for follow-up permissions
        foreach($user->roles as $role) {
            $permissions = $role->permissions->pluck('name');

            if($permissions->contains('follow-up:show-all')) {
                $hasShowAll = true;
            ...
            if($permissions->contains('follow-up:show-branch')) {
                $hasShowBranch = true;
            }
        }
